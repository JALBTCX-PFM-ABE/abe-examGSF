...
otherRecs::otherRecs (QWidget * parent, int32_t gsfHandle, OPTIONS *op, MISC *mi):
  QDialog (parent, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  start_nsec = (uint64_t) sec * 1000000000 + (uint64_t) nsec;

  gsfIndexTime (gsf_handle, GSF_RECORD_SWATH_BATHYMETRY_PING, numrecs, &sec, &nsec);
  end_nsec = (uint64_t) sec * 1000000000 + (uint64_t) nsec;
  ...
  digits = (int32_t) (log10 ((double) numothr)) + 1;
  ...
  rid = (int32_t *) calloc (numothr, sizeof (int32_t));
  rec = (int32_t *) calloc (numothr, sizeof (int32_t));
  ...
  int32_t width = 0;
  for (int32_t i = 0 ; i < 4 ; i++) width += otherTable->columnWidth (i);
  width = qMin (800, width + 60);

  int32_t height = otherTable->rowHeight (0) * numothr;
...
uint8_t 
otherRecs::readRecord (int32_t dataID, int32_t rec_num, gsfDataID *gsf_data_id, gsfRecords *gsf_record)
{
  ...
  if (gsfRead (gsf_handle, dataID, gsf_data_id, gsf_record, NULL, 0) < 0) 
    {
      ...
      errmsg.sprintf (tr ("Error reading record %d : %s").toLatin1 (), rec_num, gsfStringError ());
...
void 
otherRecs::populateTable (int32_t row)
{
...
otherRecs::slotExtend (int id)
{
  ...
  int32_t year, day, hour, minute, month, mday, i;
  float second;
  ...
  tmp.sprintf (tr (" Record %d").toLatin1 (), rec[id]);
  ...
  switch (rid[id])
    ...
    case GSF_RECORD_SOUND_VELOCITY_PROFILE:
      ...
      tmp.sprintf (tr ("<i>Observation Time:</i> %d-%02d-%02d (%03d) %02d:%02d:%05.2f").toLatin1 (), year + 1900, month, 
      ...
      tmp.sprintf (tr ("<i>Application Time:</i> %d-%02d-%02d (%03d) %02d:%02d:%05.2f").toLatin1 (), year + 1900, month, 
                   mday, day, hour, minute, second);

      tEdit->append (tmp);


      tmp.sprintf (tr ("<i>Number of points:</i> %d").toLatin1 (), gsf_record.svp.number_points);
      ...
      for (i = 0 ; i < sv_length ; i++)
        {
          svData[i].x = (double) gsf_record.svp.sound_speed[i];
          svData[i].y = (double) gsf_record.svp.depth[i] * misc->depth_factor[options->depth_units];
          ...
          tmp.sprintf (tr ("<i>Point:</i> %d   <i>Depth :</i>   %.2f   <i>Sound Speed:</i> %.2f").toLatin1 (), i, 
    ...
    case GSF_RECORD_PROCESSING_PARAMETERS:
      ...
      tmp.sprintf (tr ("<i>Time:</i> %d-%02d-%02d (%03d) %02d:%02d:%05.2f").toLatin1 (), year + 1900, month, mday, day,
                   hour, minute, second);

      tEdit->append (tmp);


      tmp.sprintf (tr ("<i>Number of parameters:</i> %d").toLatin1 (), gsf_record.process_parameters.number_parameters);
    ...
    case GSF_RECORD_SENSOR_PARAMETERS:
      ...
      tmp.sprintf (tr ("<i>Number of parameters:</i> %d").toLatin1 (), gsf_record.sensor_parameters.number_parameters);
    ...
    case GSF_RECORD_HISTORY:
      ...
      tmp.sprintf (tr ("<i>Host Name:</i>  %s").toLatin1 (), gsf_record.history.host_name);

      tEdit->append (tmp);

      tmp.sprintf (tr ("<i>Operator Name:</i>  %s").toLatin1 (), gsf_record.history.operator_name);

      tEdit->append (tmp);

      tmp.sprintf (tr ("<i>Command:</i> %s").toLatin1 (), gsf_record.history.command_line);

      tEdit->append (tmp);

      tmp.sprintf (tr ("<i>Comment:</i> %s").toLatin1 (), gsf_record.history.comment);
    ...
    case GSF_RECORD_NAVIGATION_ERROR:
      ...
      tmp.sprintf (tr ("Latitude Error: %09.3f").toLatin1 (), gsf_record.nav_error.latitude_error);

      tEdit->append (tmp);

      tmp.sprintf (tr ("Longitude Error: %09.3f").toLatin1 (), gsf_record.nav_error.longitude_error);
    ...
    case GSF_RECORD_SWATH_BATHY_SUMMARY:
      ...
      tmp.sprintf (tr ("<i>Start Time:</i> %d-%02d-%02d (%03d) %02d:%02d:%05.2f").toLatin1 (), year + 1900, month, 
                   mday, day, hour, minute, second);
      ...
      tmp.sprintf (tr ("<i>Maximum Depth :</i> %0.2f").toLatin1 (), gsf_record.summary.max_depth * 
                   misc->depth_factor[options->depth_units]);
      tmp.insert (17, misc->depth_string[options->depth_units]);
      tEdit->append (tmp);
    ...
    case GSF_RECORD_HV_NAVIGATION_ERROR:
      ...
      if (gsf_record.hv_nav_error.horizontal_error == GSF_NULL_HORIZONTAL_ERROR)
        {
          tmp = tr ("<i>Horizontal Error: Unknown</i>");
        }
      else
        {
          tmp.sprintf ("<i>Horizontal Error:</i> %09.3f", gsf_record.hv_nav_error.horizontal_error);
        }
      ...
      if (gsf_record.hv_nav_error.vertical_error == GSF_NULL_HORIZONTAL_ERROR)
        {
          tmp = tr ("<i>Vertical Error: Unknown</i>");
        }
      else
        {
          tmp.sprintf ("<i>Vertical Error :</i> %09.3f", gsf_record.hv_nav_error.vertical_error * misc->depth_factor[options->depth_units]);
      ...
      tmp.sprintf (tr ("<i>Position Type:</i> %s").toLatin1 (), gsf_record.hv_nav_error.position_type);
...
void
otherRecs::plotSvProfile (int32_t recnum)
{
  ...
  connect (sv_profile, SIGNAL (mouseMoveSignal (QMouseEvent *, double, double)), this, 
           SLOT (slotSvProfileMouseMove (QMouseEvent *, double, double)));
...
void 
otherRecs::slotSvProfileMouseMove (QMouseEvent *e __attribute__ ((unused)), double x, double y)
{
  QString tmp;
