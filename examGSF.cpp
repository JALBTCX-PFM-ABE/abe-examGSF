...
examGSF::examGSF (int32_t *argc, char **argv, QWidget * parent):
  QMainWindow (parent, 0)
{
  ...
  setWindowIcon (QIcon (":/icons/examGSF.png"));
  ...
  bOther->setIcon (QIcon (":/icons/other.png"));
...
examGSF::slotOpen ()
{
  ...
  filters << tr ("GSF (*.d[0-9][0-9] *.gsf)")
          << tr ("All files (*)");

  fd->setNameFilters (filters);
  fd->setFileMode (QFileDialog::ExistingFile);
  fd->selectNameFilter (tr ("GSF (*.d[0-9][0-9] *.gsf)"));
...
examGSF::openFile ()
{
  ...
  if ((fp = fopen (ndx_file.toLatin1 (), "r")) == NULL)
    {
      ...
      if (gsfOpen (filename.toLatin1 (), GSF_READONLY_INDEX, &gsf_handle)) 
        {
          QString errmsg;

          errmsg.sprintf (tr ("Unable to index GSF file : %s").toLatin1 (), gsfStringError ());

          QMessageBox::warning (this, tr ("Open GSF File"), errmsg);
  ...
  connect (other_recs, SIGNAL (findPingSignal (int32_t)), this, SLOT (slotFindPing (int32_t)));
  ...
  if (gsfRead (gsf_handle, GSF_RECORD_SWATH_BATHYMETRY_PING, &gsf_data_id, &gsf_record, NULL, 0) < 0) 
    {
      ...
      errmsg.sprintf (tr ("Error reading record 1 : %s").toLatin1 (), gsfStringError ());
...
void 
examGSF::readData (int32_t record)
{
  extern int gsfError;
      ...
      if (gsfRead (gsf_handle, GSF_RECORD_SWATH_BATHYMETRY_PING, &gsf_data_id, &gsf_record, NULL, 0) < 0) 
        {
          ...
          errmsg.sprintf (tr ("Error reading record %d : %s").toLatin1 (), i, gsfStringError ());
  ...
  int32_t width = 0;
  for (int32_t i = 0 ; i < GSF_COLUMNS ; i++) width += gsfTable->columnWidth (i);
  width = qMin (1200, width + 120);

  int32_t height = gsfTable->rowHeight (0) * PAGE_SIZE;
...
void 
examGSF::populateTable (int32_t row, gsfDataID dataID, gsfRecords record)
{
  char hem, format[5];
  double deg, min, sec;
  int32_t cnt = 0;
  ...
  cnt = 0;
  for (int32_t i = 0 ; i < record.mb_ping.number_beams ; i++)
    {
  ...
  int32_t tide_type = (record.mb_ping.ping_flags & (GSF_PING_USER_FLAG_15 | GSF_PING_USER_FLAG_14)) >> 14;
  ...
  switch (record.mb_ping.sensor_id)
    ...
    case GSF_SWATH_BATHY_SUBRECORD_SEABAT_8101_SPECIFIC:
      ...
      tmp.sprintf ("%5.2f", (double) record.mb_ping.sensor_data.gsfSeaBat8101Specific.tvg_spreading * 0.25);
      gsfTable->setItem (row, GSF_COLUMNS + 7, new QTableWidgetItem (tmp));
    ...
    case GSF_SWATH_BATHY_SUBRECORD_RESON_7125_SPECIFIC:
      ...
      gsfTable->setHorizontalHeaderItem (GSF_COLUMNS + 14,
                                         new QTableWidgetItem (tr ("Time Varying\nGain Spreading\nCoefficient")));
      tmp.sprintf ("%.2f", (float) gsf_record.mb_ping.sensor_data.gsfReson8100Specific.tvg_spreading / 4.0);
      gsfTable->setItem (row, GSF_COLUMNS + 14, new QTableWidgetItem (tmp));
      ...
      tmp.sprintf ("%f", (float) gsf_record.mb_ping.sensor_data.gsfReson8100Specific.projector_angle / 10.0);
      gsfTable->setItem (row, GSF_COLUMNS + 19, new QTableWidgetItem (tmp));
      ...
      tmp.sprintf ("%f", (float) gsf_record.mb_ping.sensor_data.gsfReson8100Specific.temperature / 10.0);
      gsfTable->setItem (row, GSF_COLUMNS + 25, new QTableWidgetItem (tmp));
...
void 
examGSF::slotFindPing (int32_t record)
{
  record_num = record;
  scrollBar->setValue (record_num);
...
examGSF::slotScrollBarSliderMoved (int value)
{
  ...
  QString tmp;
  tmp.sprintf (tr ("Record: %d    Date/Time: %d-%02d-%02d (%03d) %02d:%02d:%05.2f").toLatin1 (), value, year + 1900,
...
void 
examGSF::slotDataProfileMouseMove (QMouseEvent *e __attribute__ ((unused)), double x, double y)
{
  QString tmp;
...
examGSF::envin ()
{
  ...
  misc.depth_string[0] = QString ("(meters)");
  misc.depth_factor[0] = 1.0;
  misc.depth_string[1] = QString ("(feet)");
  misc.depth_factor[1] = 3.28084;
  misc.depth_string[2] = QString ("(fathoms)");
  misc.depth_factor[2] = 0.54681;
  misc.depth_string[3] = QString ("(cubits)");
  misc.depth_factor[3] = 0.44196;
  misc.depth_string[4] = QString ("(willetts)");
...
#ifdef NVWIN3X
  QString ini_file = QString (getenv ("USERPROFILE")) + "/ABE.config/examGSF.ini";
#else
  QString ini_file = QString (getenv ("HOME")) + "/ABE.config/examGSF.ini";
#endif

  QSettings settings (ini_file, QSettings::IniFormat);
  settings.beginGroup ("examGSF");
  ...
  options.window_x = settings.value (QString ("window x"), options.window_x).toInt ();

  options.window_y = settings.value (QString ("window y"), options.window_y).toInt ();


  options.beam_width = settings.value (QString ("beam width"), options.beam_width).toInt ();

  options.beam_height = settings.value (QString ("beam height"), options.beam_height).toInt ();

  options.beam_window_x = settings.value (QString ("beam window x"), options.beam_window_x).toInt ();

  options.beam_window_y = settings.value (QString ("beam window y"), options.beam_window_y).toInt ();


  options.other_width = settings.value (QString ("other width"), options.other_width).toInt ();

  options.other_height = settings.value (QString ("other height"), options.other_height).toInt ();

  options.other_window_x = settings.value (QString ("other window x"), options.other_window_x).toInt ();

  options.other_window_y = settings.value (QString ("other window y"), options.other_window_y).toInt ();


  options.extended_width = settings.value (QString ("extended width"), options.extended_width).toInt ();

  options.extended_height = settings.value (QString ("extended height"), options.extended_height).toInt ();

  options.extended_window_x = settings.value (QString ("extended window x"), options.extended_window_x).toInt ();

  options.extended_window_y = settings.value (QString ("extended window y"), options.extended_window_y).toInt ();


  options.profile_width = settings.value (QString ("profile width"), options.profile_width).toInt ();

  options.profile_height = settings.value (QString ("profile height"), options.profile_height).toInt ();

  options.profile_window_x = settings.value (QString ("profile window x"), options.profile_window_x).toInt ();

  options.profile_window_y = settings.value (QString ("profile window y"), options.profile_window_y).toInt ();


  options.svp_width = settings.value (QString ("svp width"), options.svp_width).toInt ();

  options.svp_height = settings.value (QString ("svp height"), options.svp_height).toInt ();

  options.svp_window_x = settings.value (QString ("svp window x"), options.svp_window_x).toInt ();

  options.svp_window_y = settings.value (QString ("svp window y"), options.svp_window_y).toInt ();


  options.depth_units = settings.value (QString ("depth units"), options.depth_units).toInt ();
  ...
  switch (options.depth_units)
    {
    case 1:
      QMessageBox::warning (this, QString ("Units of Measure"), feetText);
      break;

    case 2:
      QMessageBox::warning (this, QString ("Units of Measure"), fathomText);
      break;

    case 3:
      QMessageBox::warning (this, QString ("Units of Measure"), cubitText);
      break;

    case 4:
      QMessageBox::warning (this, QString ("Units of Measure"), willettText);
      break;
    }
...
examGSF::envout ()
{
  ...
  settings.setValue (QString ("window x"), options.window_x);

  settings.setValue (QString ("window y"), options.window_y);


  settings.setValue (QString ("beam width"), options.beam_width);

  settings.setValue (QString ("beam height"), options.beam_height);

  settings.setValue (QString ("beam window x"), options.beam_window_x);

  settings.setValue (QString ("beam window y"), options.beam_window_y);


  settings.setValue (QString ("other width"), options.other_width);

  settings.setValue (QString ("other height"), options.other_height);

  settings.setValue (QString ("other window x"), options.other_window_x);

  settings.setValue (QString ("other window y"), options.other_window_y);


  settings.setValue (QString ("extended width"), options.extended_width);

  settings.setValue (QString ("extended height"), options.extended_height);

  settings.setValue (QString ("extended window x"), options.extended_window_x);

  settings.setValue (QString ("extended window y"), options.extended_window_y);


  settings.setValue (QString ("profile width"), options.profile_width);

  settings.setValue (QString ("profile height"), options.profile_height);

  settings.setValue (QString ("profile window x"), options.profile_window_x);

  settings.setValue (QString ("profile window y"), options.profile_window_y);


  settings.setValue (QString ("svp width"), options.svp_width);

  settings.setValue (QString ("svp height"), options.svp_height);

  settings.setValue (QString ("svp window x"), options.svp_window_x);

  settings.setValue (QString ("svp window y"), options.svp_window_y);


  settings.setValue (QString ("depth units"), options.depth_units);
