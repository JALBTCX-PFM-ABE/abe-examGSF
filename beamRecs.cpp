...
beamRecs::beamRecs (QWidget * parent, int32_t gsfHandle, int32_t num_ping, OPTIONS *op, MISC *mi):
...
{
  int32_t numrecs = gsfGetNumberRecords (gsf_handle, GSF_RECORD_SWATH_BATHYMETRY_PING);
  ...
  tmp.sprintf (tr ("examGSF Beam Data - Pings %d, %d, %d").toLatin1 (), start, start + 1, end);
  setWindowTitle (tmp);
  ...
  //  Pre-read all three records to get the number of beams (they may be different).
  ...
  for (i = start ; i <= end ; i++)
    {
      ...
      max_beams = qMax (max_beams, gsf_record.mb_ping.number_beams);
    }
  ...
  beamTable = new QTableWidget (62, max_beams, this);
  ...
  for (i = start ; i <= end ; i++)
    {
      ...
      for (int32_t rownum = 0 ; rownum < NUM_ROWS ; rownum++)
              ...
              for (n = 0 ; n < beams ; n++)
                {
                  switch (m)
                    ...
                    case 1:
                      tmp.sprintf ("%0.3f", gsf_record.mb_ping.nominal_depth[n] * misc->depth_factor[options->depth_units]);
                      break;
...
void
beamRecs::slotProfile (int32_t logicalIndex)
{
  ...
  int32_t rec = start + logicalIndex / NUM_ROWS;
  QString tmp;
  ...
  double *dataPtr;
  int32_t ndx = (logicalIndex - logicalIndex / NUM_ROWS) % NUM_ARRAYS;
  ...
  for (int32_t i = 0 ; i < beams ; i++)
    {
      data[i].x = (double) i;

      if (check_flag (gsf_record.mb_ping.beam_flags[i], NV_GSF_IGNORE_NULL_BEAM) ||
          check_flag (gsf_record.mb_ping.beam_flags[i], NV_GSF_IGNORE_MANUALLY_EDITED) ||
          check_flag (gsf_record.mb_ping.beam_flags[i], NV_GSF_IGNORE_FILTER_EDITED))
        {
  ...
  connect (data_profile, SIGNAL (mouseMoveSignal (QMouseEvent *, double, double)), this, 
           SLOT (slotDataProfileMouseMove (QMouseEvent *, double, double)));
...
void 
beamRecs::slotDataProfileMouseMove (QMouseEvent *e __attribute__ ((unused)), double x, double y)
{
  QString tmp;
